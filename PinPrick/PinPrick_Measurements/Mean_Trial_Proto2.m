...
%% Parameters
SeuilPic = 0.15; %valeur de seuil de detection de pic
SeuilPicF = 0.15; 
...
F1= importdata('manual_German_Trial1_0.csv');
Fz1 = -F1.data(:,9);
X1 = 1:length(Fz1);

F2= importdata('manual_BN_Trial1_0.csv');
Fz2 = -F2.data(:,9);
X2 = 1:length(Fz2);
...
%  ValidCycles2([2 18 27 44 45 58])=[];
%  CyclesForMean2([3 4 35 36 53 54 87 88 89 90 115 116])=[];
% % remove outlayers peaks [1 2 12 29 30 51 56]
%  ValidCycles2([5 8 23 24 25 26 28 30 31 32 34 37 38 39 41 42 43])=[];
%  CyclesForMean2([9 10 15 16 45 46 47 48 49 50 51 52 55 56 59 60 61 62 63 64 67 68 73 74 75 76 77 78 81 82 83 84 85 86]) =[];
...
% ValidPics2([2 18 27 44 45 58])=[];
% % remove outlayers peaks
%  ValidPics2([5 8 23 24 25 26 28 30 31 32 34 37 38 39 41 42 43])=[];
