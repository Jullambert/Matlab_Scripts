...
switch VoltagePP
    ...
    case 1
        DistanceTransducteurHydrophone = 5.6; %4.64; %5.4; %[mm] 5.6 pour nouvelle pièce d'alignement
        switch UltrasoundBurstFrequency 
            case 250000
                HydrophoneSensitivity = 0.84;
                PeakThreshold_Hydrophone = 2400/1000000; %2700/1000000; %50/1000000;3300/1000000;
            case 350000
                HydrophoneSensitivity = 0.99;
...
for iii=1:length(NameFile2)
    ...
    [a_2,b_2]=sort(stval_2);
        ...
        if stval_2(iii) == 250000
            if mod(iii,2)
                movefile(NameFile2{iii},'E:\Documents\Dropbox (NOCIONS)\jlambert\UCL_PhD\Setup\Mesures_Welcome\SessionNovembre_ValidationStimulateur\AnciennePiece_Validation3\250kHz')                
            else
                movefile(NameFile2{iii},'E:\Documents\Dropbox (NOCIONS)\jlambert\UCL_PhD\Setup\Mesures_Welcome\SessionNovembre_ValidationStimulateur\AnciennePiece_Validation3\250kHz_index')
            end
        elseif stval_2(iii) == 350000            
            if mod(iii,2)
                movefile(NameFile2{iii},'E:\Documents\Dropbox (NOCIONS)\jlambert\UCL_PhD\Setup\Mesures_Welcome\SessionNovembre_ValidationStimulateur\AnciennePiece_Validation3\350kHz')                
            else
                movefile(NameFile2{iii},'E:\Documents\Dropbox (NOCIONS)\jlambert\UCL_PhD\Setup\Mesures_Welcome\SessionNovembre_ValidationStimulateur\AnciennePiece_Validation3\350kHz_index')
            end
        else        
            if mod(iii,2)
                movefile(NameFile2{iii},'E:\Documents\Dropbox (NOCIONS)\jlambert\UCL_PhD\Setup\Mesures_Welcome\SessionNovembre_ValidationStimulateur\AnciennePiece_Validation3\500kHz')                
            else
                movefile(NameFile2{iii},'E:\Documents\Dropbox (NOCIONS)\jlambert\UCL_PhD\Setup\Mesures_Welcome\SessionNovembre_ValidationStimulateur\AnciennePiece_Validation3\500kHz_index')
            end
        end
...
%% 8 Analyse fréquentielle des données. 
...
FFT_Dephasage = angle(Burst_Tension_Hydro_Resampled./Burst_Tension_FunGen_Resampled); %= dephasage entre émis et reçu cfr notes
FFT_Amplitude =  abs(Burst_Tension_Hydro_Resampled./Burst_Tension_FunGen_Resampled);

disp('8 : Analyse FFT finie')
...
% filenametosave = strcat('Resultats_Analyse_Data_SansCrane_ResolSpat1mm_',num2str(NumberOfCycles),'Cycles_0',num2str(VoltagePP*10),'Vpp_',num2str(UltrasoundBurstFrequency/1000),'_kHz','.mat');
% % clear(VariableNameList_Original{:})
% save(filenametosave)
NameData=DataFilesName(1).folder;
idx_session=strfind(st,'_Welcome\')+9;
idx2_session=strfind(st,'Crane')-6;
session = NameData(idx_session:idx2_session);

idx_type=strfind(st,'Crane\')-4;
idx2_type=strfind(st,'Vpp')-4;
...
clear DistanceDerating_Isppa IsppaDerated LinearAttenuationCoeff DistanceDerating_Ispta IsptaDerated LinearAttenuationCoeff DistanceDerating_MI MIDerated LinearAttenuationCoeff
...
figure;
subplot(3,1,1);
h1=plot(squeeze(Tension_Hydrophone(19,101,:)),'k');
xlim([10000 20000]);
ylim([-0.5 0.5]);
subplot(3,1,2);
h2=plot(squeeze(Tension_Hydrophone(27,101,:)),'k');
xlim([10000 20000]);
ylim([-0.5 0.5]);
subplot(3,1,3);
h3=plot(squeeze(Tension_Hydrophone(35,101,:)),'k');
xlim([10000 20000]);
ylim([-0.5 0.5]);
for i=101:-1:1;
    set(h1,'YData',squeeze(Tension_Hydrophone(24,i,:)));
    set(h2,'YData',squeeze(Tension_Hydrophone(27,i,:)));
    set(h3,'YData',squeeze(Tension_Hydrophone(30,i,:)));
    pause(0.25);
end;
% 
% 
