...
elseif UltrasoundBurstFrequency == 350000
...
disp('Reorganisation des noms de fichiers fini')
...
for iii=1:length(NameFile2);
    ...
    if stval_2(iii) == 250000
        movefile(NameFile2{iii},'C:\Users\jullambert\Dropbox (NOCIONS)\jlambert\UCL_PhD\Setup\Mesures_Welcome\SessionMars_Avril\Mesures_Mars\SansCrane\250kHz')
       
    elseif stval_2(iii) == 350000
        
        movefile(NameFile2{iii},'C:\Users\jullambert\Dropbox (NOCIONS)\jlambert\UCL_PhD\Setup\Mesures_Welcome\SessionMars_Avril\Mesures_Mars\SansCrane\350kHz')
        
    else        
        movefile(NameFile2{iii},'C:\Users\jullambert\Dropbox (NOCIONS)\jlambert\UCL_PhD\Setup\Mesures_Welcome\SessionMars_Avril\Mesures_Mars\SansCrane\500kHz')
    end
 end;

 

%% Boucle permettant l'import des données dans une matrice
...
disp('Correction des valeurs en fonction de leur offset, filtrage des données et détection des premiers pics finis')
...
%% Calcul des différents paramètres "derated" nécessaires pour la caractérisation d'un champs d'ultrasons
...
disp('Calcul des parametres "derated" du champs d ultrasons fini')

%% Calcul de la Full width half maximum area
...
VariableNameList{ww,1} = strcat(VariableName(ww),'_',num2str(NumberOfCycles),'Cycles_',num2str(VoltagePP),'Vpp_',num2str(UltrasoundBurstFrequency/1000),'_kHz');
end

save(strcat('Resultats_Analyse_Data_',num2str(NumberOfCycles),'Cycles_',num2str(VoltagePP),'Vpp_',num2str(UltrasoundBurstFrequency/1000),'_kHz'))
