...
SamplingFrequency = 60000000;%pour 200 cycles %60000000; %pour 5 cycles
CutoffFrequency = 1000000;
NumberOfCycles = 5; % 5
SonicationDuration = 0.3 ;% time in second
NumberSamplesRecorded =  20000;%20000; %pour 5 cycles
...
switch VoltagePP
    ...
    case 1
        DistanceTransducteurHydrophone = 5.7; %(proto_Zero_Protection %10.6; %(protoMoinsDix)%5.4;%(pour proto Zero)%5.5 ;%(pour proto hors)%5.6;%(pour 20 Deg)%5.54; %%(pour10Deg)%5.6; %(pour5Deg);%5.3; %(pour-10)%5.24;%(pour0Deg) ;%5.28;%(pour -5deg)%5.54; %%(pour10Deg) %4.64; %5.4; %[mm] 5.6 pour nouvelle pièce d'alignement
        switch UltrasoundBurstFrequency 
            case 250000
                HydrophoneSensitivity = 0.84;
                PeakThreshold_Hydrophone = 3380/1000000; %2700/1000000; %50/1000000;3300/1000000;
            case 350000
                HydrophoneSensitivity = 0.99;
                PeakThreshold_Hydrophone = 4000/1000000; %2800 pr 5 cycles
            case 500000
                HydrophoneSensitivity = 1.22;
                PeakThreshold_Hydrophone = 3500/1000000; %2300 pour 5 cycles
        end
    case 0.8
...
for iii=1:length(NameFile2)
    ...
    stval_2(iii)=str2num(st(idx_2(1):idx2_2));
    [a_2,b_2]=sort(stval_2);

        if stval_2(iii) == 250000
            if mod(iii,2)
                movefile(NameFile2{iii},'E:\data\jlambert\TFUS_Mesures_Welcome\SessionJanvier\Proto_Tr_Zero_Protection\SansCrane\250kHz')                
            else
                movefile(NameFile2{iii},'E:\data\jlambert\TFUS_Mesures_Welcome\SessionJanvier\Proto_Tr_Zero_Protection\SansCrane\250kHz_index')
            end
        elseif stval_2(iii) == 350000            
            if mod(iii,2)
                movefile(NameFile2{iii},'E:\data\jlambert\TFUS_Mesures_Welcome\SessionJanvier\Proto_Tr_Zero_Protection\SansCrane\350kHz')                
            else
                movefile(NameFile2{iii},'E:\data\jlambert\TFUS_Mesures_Welcome\SessionJanvier\Proto_Tr_Zero_Protection\SansCrane\350kHz_index')
            end
        else        
            if mod(iii,2)
                movefile(NameFile2{iii},'E:\data\jlambert\TFUS_Mesures_Welcome\SessionJanvier\Proto_Tr_Zero_Protection\SansCrane\500kHz')                
            else
                movefile(NameFile2{iii},'E:\data\jlambert\TFUS_Mesures_Welcome\SessionJanvier\Proto_Tr_Zero_Protection\SansCrane\500kHz_index')
            end
        end
