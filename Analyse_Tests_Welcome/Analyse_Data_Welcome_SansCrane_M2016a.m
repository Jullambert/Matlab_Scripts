...
SamplingFrequency = 10000000;%10000000;%pour 200 cycles %60000000; %pour 5 cycles 2mm resol spatiale
CutoffFrequency = 1000000;
NumberOfCycles = 5; % 5
SonicationDuration = 0.3 ;% time in second
NumberSamplesRecorded = 30000; %30000; %pour 5 cycles
...
%         end
    ...
    case 1
        DistanceTransducteurHydrophone = 5.62; %5.6;(SessionJuillet Rotation -30, +15Deg_bis)%6.34;%(SessionAttenuation 20 cycles_1)%5.3; % ( SessionAttenuation 5)% 5.2;%(SessionAttenuation4 SansCrane)%5.74; %(SessionAttenuation3 SansCrane) 5.1; %(SessionAttenuation2 SansCrane)%5.6; %(SessionAttenuation1 SansCrane) %5.7; %(proto_Zero_Protection %10.6; %(protoMoinsDix)%5.4;%(pour proto Zero)%5.5 ;%(pour proto hors)%5.6;%(pour 20 Deg)%5.54; %%(pour10Deg)%5.6; %(pour5Deg);%5.3; %(pour-10)%5.24;%(pour0Deg) ;%5.28;%(pour -5deg)%5.54; %%(pour10Deg) %4.64; %5.4; %[mm] 5.6 pour nouvelle pièce d'alignement
        switch UltrasoundBurstFrequency 
            case 250000
                HydrophoneSensitivity = 0.84;
                PeakThreshold_Hydrophone = 3750/1000000; %2700/1000000; %50/1000000;3300/1000000;
            case 300000
                HydrophoneSensitivity = 0.91;
                PeakThreshold_Hydrophone = 3000/1000000; %
            case 350000
                HydrophoneSensitivity = 0.99;
                PeakThreshold_Hydrophone = 3800/1000000; %2800 pr 5 cycles
            case 400000
                HydrophoneSensitivity = 1.07;
                PeakThreshold_Hydrophone = 4200/1000000; %
            case 450000
            ...
            case 500000
                HydrophoneSensitivity = 1.22;
                PeakThreshold_Hydrophone = 3500/1000000; % 3500/1000000;2300 pour 5 cycles
        end
    case 0.8
...
for iii=1:length(NameFile2)
    ...
    [a_2,b_2]=sort(stval_2);
        ...
        if stval_2(iii) == 250000
            if mod(iii,2)
                movefile(NameFile2{iii},'D:\data\jlambert\TFUS_Mesures_Welcome\SessionJuillet_Rotations\250kHz')                
            else
                movefile(NameFile2{iii},'D:\data\jlambert\TFUS_Mesures_Welcome\SessionJuillet_Rotations\250kHz_index')
            end
            
        elseif stval_2(iii) == 300000            
            if mod(iii,2)
                movefile(NameFile2{iii},'D:\data\jlambert\TFUS_Mesures_Welcome\SessionJuillet_Rotations\300kHz')                
            else
                movefile(NameFile2{iii},'D:\data\jlambert\TFUS_Mesures_Welcome\SessionJuillet_Rotations\300kHz_index')
            end
        elseif stval_2(iii) == 350000            
            if mod(iii,2)
                movefile(NameFile2{iii},'D:\data\jlambert\TFUS_Mesures_Welcome\SessionJuillet_Rotations\350kHz')                
            else
                movefile(NameFile2{iii},'D:\data\jlambert\TFUS_Mesures_Welcome\SessionJuillet_Rotations\350kHz_index')
            end
        elseif stval_2(iii) == 400000            
            if mod(iii,2)
                movefile(NameFile2{iii},'D:\data\jlambert\TFUS_Mesures_Welcome\SessionJuillet_Rotations\400kHz')                
            else
                movefile(NameFile2{iii},'D:\data\jlambert\TFUS_Mesures_Welcome\SessionJuillet_Rotations\400kHz_index')
            end
        elseif stval_2(iii) == 450000            
            if mod(iii,2)
                movefile(NameFile2{iii},'D:\data\jlambert\TFUS_Mesures_Welcome\SessionJuillet_Rotations\450kHz')                
            else
                movefile(NameFile2{iii},'D:\data\jlambert\TFUS_Mesures_Welcome\SessionJuillet_Rotations\450kHz_index')
            end
        else        
            if mod(iii,2)
                movefile(NameFile2{iii},'D:\data\jlambert\TFUS_Mesures_Welcome\SessionJuillet_Rotations\500kHz')                
            else
                movefile(NameFile2{iii},'D:\data\jlambert\TFUS_Mesures_Welcome\SessionJuillet_Rotations\500kHz_index')
            end
        end
end
...
idx_session=strfind(st,'Session');
idx2_session=strfind(st,'_Rotations')-1;
% idx2_session=strfind(st,'Ancienne')-2;
session = NameData(idx_session:idx2_session);

idx_type=strfind(st,'_Rotations')+1;
idx2_type=strfind(st,'Plus')-2;
type = NameData(idx_type:idx2_type);

idx_fichier=strfind(st,'Plus');
idx2_fichier=strfind(st,'kHz')-5;
fichier = NameData(idx_fichier:idx2_fichier);

% idx_freq=strfind(st,'mm')+8;
idx_freq=strfind(st,'Deg')+4;
idx2_freq=strfind(st,'kHz')-1;
