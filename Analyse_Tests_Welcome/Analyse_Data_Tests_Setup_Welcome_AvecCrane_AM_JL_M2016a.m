...
switch VoltagePP
    ...
    case 1
        DistanceTransducteurHydrophone = 11.34;%(SessionAttenuation 20 cycles_1) %10.3;% (SessionAttenuation5 AvecCrane)%10.2;%(SessionAttenuation4 AvecCrane)%10.74; %(SessionAttenuation3 AvecCrane) %10.1; %(SessionAttenuation2 AvecCrane) %10.6; %(SessionAttenuation1 AvecCrane) %5.7; %(proto_Zero_Protection %10.6; %(protoMoinsDix)%5.4;%(pour proto Zero)%5.5 ;%(pour proto hors)%5.6;%(pour 20 Deg)%5.54; %%(pour10Deg)%5.6; %(pour5Deg);%5.3; %(pour-10)%5.24;%(pour0Deg) ;%5.28;%(pour -5deg)%5.54; %%(pour10Deg) %4.64; %5.4; %[mm] 5.6 pour nouvelle pièce d'alignement
        switch UltrasoundBurstFrequency 
            case 250000
                HydrophoneSensitivity = 0.84;
                PeakThreshold_Hydrophone = 3600/1000000; %2700/1000000; %50/1000000;3300/1000000;
            case 300000
            ...
            case 400000
                ...
                PeakThreshold_Hydrophone = 3300/1000000; %
            case 450000
                HydrophoneSensitivity = 1.14;
                PeakThreshold_Hydrophone = 2800/1000000; %
            case 500000
                HydrophoneSensitivity = 1.22;
                PeakThreshold_Hydrophone = 2650/1000000;%3050/1000000;%(sessionAttenuation_Crane2) % 3500/1000000;2300 pour 5 cycles
        end
    case 0.8
...
for iii=1:length(NameFile2)
    ...
    [a_2,b_2]=sort(stval_2);
        ...
        if stval_2(iii) == 250000
            if mod(iii,2)
                movefile(NameFile2{iii},'D:\data\jlambert\TFUS_Mesures_Welcome\SessionAttenuation\AvecCrane\250kHz')                
            else
                movefile(NameFile2{iii},'D:\data\jlambert\TFUS_Mesures_Welcome\SessionAttenuation\AvecCrane\250kHz_index')
            end
        elseif stval_2(iii) == 300000            
            if mod(iii,2)
                movefile(NameFile2{iii},'D:\data\jlambert\TFUS_Mesures_Welcome\SessionAttenuation\AvecCrane\300kHz')                
            else
                movefile(NameFile2{iii},'D:\data\jlambert\TFUS_Mesures_Welcome\SessionAttenuation\AvecCrane\300kHz_index')
            end
        elseif stval_2(iii) == 350000            
            if mod(iii,2)
                movefile(NameFile2{iii},'D:\data\jlambert\TFUS_Mesures_Welcome\SessionAttenuation\AvecCrane\350kHz')                
            else
                movefile(NameFile2{iii},'D:\data\jlambert\TFUS_Mesures_Welcome\SessionAttenuation\AvecCrane\350kHz_index')
            end
        elseif stval_2(iii) == 400000            
            if mod(iii,2)
                movefile(NameFile2{iii},'D:\data\jlambert\TFUS_Mesures_Welcome\SessionAttenuation\AvecCrane\400kHz')                
            else
                movefile(NameFile2{iii},'D:\data\jlambert\TFUS_Mesures_Welcome\SessionAttenuation\AvecCrane\400kHz_index')
            end
        elseif stval_2(iii) == 450000            
            if mod(iii,2)
                movefile(NameFile2{iii},'D:\data\jlambert\TFUS_Mesures_Welcome\SessionAttenuation\AvecCrane\450kHz')                
            else
                movefile(NameFile2{iii},'D:\data\jlambert\TFUS_Mesures_Welcome\SessionAttenuation\AvecCrane\450kHz_index')
            end
        else        
            if mod(iii,2)
                movefile(NameFile2{iii},'D:\data\jlambert\TFUS_Mesures_Welcome\SessionAttenuation\AvecCrane\500kHz')                
            else
                movefile(NameFile2{iii},'D:\data\jlambert\TFUS_Mesures_Welcome\SessionAttenuation\AvecCrane\500kHz_index')
            end
        end
...
figure
plot(squeeze(Tension_Hydrophone_Corr_Filtre(x,y,NumCycle_FunctionGen(x,y,:)-100:end)))
figure
...
% % détourage de la zone FWHM - essai du 19/10/2016 --> KO

% %Selon les lignes
% FWHM_Matrix_2 = zeros(size(v,1),size(v,2));
% for x=1:size(v,1)
%     RowSum = cumsum(FWHM_Matrix(x,:));
%     if RowSum(end)~=0
%         for y=2:size(v,2)
%             ColSum = cumsum(FWHM_Matrix(:,y));
%             if ColSum(end)~=0
%             LeftSum = cumsum(FWHM_Matrix(x,1:y-1));
%             RightSum = cumsum(FWHM_Matrix(x,y:end));
% 
%                 if LeftSum(end)==0
%                     if RightSum(end) > 3
%                         FWHM_Matrix_2(x,y) = 0.5;
%                     end
%                 end
%             clear LeftSum
%             clear RightSum
...
%             RowSum = cumsum(FWHM_Matrix(x,:));
%             if RowSum(end)~=0
%             UpperSum = cumsum(FWHM_Matrix(1:x-1,y));
%             LowerSum = cumsum(FWHM_Matrix(x:end,y));
% 
%                 if UpperSum(end)==0
%                     if LowerSum(end) > 3
%                         FWHM_Matrix_2(x,y) = 0.5;
...
% 
% for x= 1:size(v,1)
%     if cumsum(FWHM_Matrix_2(x,:))==0
%         Row_FWHM_2(x) = 0;
%     else
%         Row_FWHM_2(x) = x;%FWHM_Matrix_2(x,find(FWHM_Matrix_2(x,:),1));
%     end
% end
% 
% for y= 1:size(v,2)
%     if cumsum(FWHM_Matrix_2(:,y))==0
%         Col_FWHM_2(y) = 0;
%     else
%         Col_FWHM_2(y) = y;%find(FWHM_Matrix_2(:,y),1);
%     end
% end
% 
% find(FWHM_Matrix_2(:,100))
% 
% disp(['Les dimensions de la FWHM sont de ' num2str(Width_FWHM) ' mm * ' num2str(Length_FWHM) ' mm'])
% 
figure
imagesc(FWHM_Matrix)
...
figure;
subplot(3,1,1);
h1=plot(squeeze(Tension_FunctionGen(24,101,:)),'k');
xlim([xmin xmax]);
ylim([ymin ymax]);
subplot(3,1,2);
h2=plot(squeeze(Tension_FunctionGen(26,101,:)),'k');
xlim([xmin xmax]);
ylim([ymin ymax]);
subplot(3,1,3);
h3=plot(squeeze(Tension_FunctionGen(28,101,:)),'k');
xlim([xmin xmax]);
ylim([ymin ymax]);
for i=101:-1:1;
    set(h1,'YData',squeeze(Tension_FunctionGen(24,i,:)));
    set(h2,'YData',squeeze(Tension_FunctionGen(26,i,:)));
    set(h3,'YData',squeeze(Tension_FunctionGen(28,i,:)));
    pause(0.25);
end;
