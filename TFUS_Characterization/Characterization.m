...
if ~Params.CalibrationPrototype(DfN)
    % If Calibration prototype =0 : meaning that measurements were performed in transversal plan
    % If Calibration prototype =1 : meaning that measurements were
    % performed with the calibrationprotoytpe and multiple measurements per
    % folder are expected
...
if table == 2
    for x=1:length(Xvector)
       ...
       % Boucle utilisée afin de respecter le schema de déplacement "en snake" de la table
% x-y
        for x=2:2:length(Xvector)%YTableEnd;
            v(x,:)=flip(v(x,:));
        end;
end
    %  FFT Parameters
...
%             PtPPressure(x,y) = max(BurstHydrophone(x,y,round(length(BurstHydrophone(x,y,:))/2):end))-min(BurstHydrophone(x,y,round(length(BurstHydrophone(x,y,:))/2)));
    ...
    if Params.SkullBone(DfN)
        if length(Yvector)<5
            IsppaEffective = max(max(IpaEffective(:,1:size(IpaEffective,2))));
            IpaEffectiveNormalized = IpaEffective./IsppaEffective;
            PeakPressureNormalized = PeakPressure./max(max(PeakPressure(:,1:size(PeakPressure,2))));
            [PeakPressureXcoord, PeakPressureYcoord] = find(PeakPressure==max(max(PeakPressure(:,1:end))));
            disp(['La valeur max du PeakPressure est de : ' num2str(max(max(PeakPressure(:,1:end)))) ' et est postionné au point : ' num2str(PeakPressureXcoord) ' ' num2str(PeakPressureYcoord)])
            [IsppaEffXcoord, IsppaEffYcoord] = find(IpaEffective==max(max(IpaEffective(:,1:end))));
            disp(['La valeur max de l Isppa effective est de : ' num2str(max(max(IpaEffective(:,1:end)))) ' et est postionné au point : ' num2str(IsppaEffXcoord) ' ' num2str(IsppaEffYcoord)])
            [IsppaXcoord, IsppaYcoord] = find(Ipa==max(max(Ipa(:,1:end))));
            disp(['La valeur max de l Isppa est de : ' num2str(max(max(Ipa(:,1:end)))) ' et est postionné au point : ' num2str(IsppaXcoord) ' ' num2str(IsppaYcoord)])
            [IsptaXcoord, IsptaYcoord] = find(Ita==max(max(Ita(:,1:end))));
            disp(['La valeur max de l Ispta est de : ' num2str(max(max(Ita(:,1:end)))) ' et est postionné au point : ' num2str(IsptaXcoord) ' ' num2str(IsptaYcoord)])
            [MIXcoord, MIYcoord] = find(MI==max(max(MI(:,1:end))));
            disp(['La valeur max du MI est de : ' num2str(max(max(MI(:,1:end)))) ' et est postionné au point : ' num2str(MIXcoord) ' ' num2str(MIYcoord)])
            [PIIXcoord, PIIYcoord] = find(PII==max(max(PII(:,1:end))));
            disp(['La valeur max du PII est de : ' num2str(max(max(PII(:,1:end)))) ' et est postionné au point : ' num2str(PIIXcoord) ' ' num2str(PIIYcoord)])
        else
...
if Params.CalibrationPrototype(DfN)
    ...
    if Params.SkullBone(DfN)
        if length(Yvector)<5
            Results={Params.DataFilename{DfN},max(max(PeakPressure(:,1:end))),PeakPressureXcoord, PeakPressureYcoord,max(max(IpaEffective(:,1:end))),...
                IsppaEffXcoord, IsppaEffYcoord,max(max(Ipa(:,1:end))),IsppaXcoord, IsppaYcoord,max(max(Ita(:,1:end))),IsptaXcoord, ...
                IsptaYcoord,max(max(MI(:,1:end))),MIXcoord, MIYcoord,FocalPointX,FocalPointY,FWHMWidth,FWHMLength};
...
ystep = 1;
figure
subplot(5,1,1); h1 = plot(squeeze(HydrophoneVoltage(xindex-2,yindex,7500:end)));
xlim([xmin xmax]);
ylim([ymin ymax]);
subplot(5,1,2); h2 = plot(squeeze(HydrophoneVoltage(xindex-1,yindex,7500:end)));
xlim([xmin xmax]);
ylim([ymin ymax]);
subplot(5,1,3); h3 =  plot(squeeze(HydrophoneVoltage(xindex,yindex,7500:end)));
xlim([xmin xmax]);
ylim([ymin ymax]);
subplot(5,1,4); h4 = plot(squeeze(HydrophoneVoltage(xindex+1,yindex,7500:end)));
xlim([xmin xmax]);
ylim([ymin ymax]);
subplot(5,1,5); h5 = plot(squeeze(HydrophoneVoltage(xindex+2,yindex,7500:end)));
xlim([xmin xmax]);
ylim([ymin ymax]);
Yv = -10:60;
for i=size(HydrophoneVoltage,2):-1:1;
    plotname=strcat('Evolution Hydrophone measures 200 cycles 350kHz - Coordonnée Y : ', num2str(Yv(i)));
    title (plotname)
    set(h1,'YData',squeeze(HydrophoneVoltage(xindex-2,i,7500:end)));
    set(h2,'YData',squeeze(HydrophoneVoltage(xindex-1,i,7500:end)));
    set(h3,'YData',squeeze(HydrophoneVoltage(xindex,i,7500:end)));
    set(h4,'YData',squeeze(HydrophoneVoltage(xindex+1,i,7500:end)));
    set(h5,'YData',squeeze(HydrophoneVoltage(xindex+2,i,7500:end)));
    xlabel(num2str(i));
    pause(2);
end;
...
%%
figure
h1=plot(squeeze(HydrophoneVoltage(1,1,7500:end)));
for j=size(HydrophoneVoltage,2):-1:1;
    set(h1,'YData',squeeze(HydrophoneVoltage(1,j,:)));
    pause(2);
end;
